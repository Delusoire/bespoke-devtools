...
function patchElementsPanel(element, MAP_CSS_CLASS, classmapStorage, isRefresh = false) {
   ...
   async function patchSidebar(sidebar) {
      ...
      function onTabClick(e) {
         e.tab = tab;
         const selectedTab = tabs.querySelector(".tabbed-pane-header-tab.selected");
         if (selectedTab === tab) {
            return;
         }
   ...
   (async function () {
      ...
      waitFor(
         ...
         elementsViewP.promise,
         (elementsViewWrapper) => {
            elementsViewWrapper.querySelectorAll(".webkit-html-attribute").forEach(
   ...
   (async function () {
      ...
      waitFor(
         () => sidebar.querySelector(".style-panes-wrapper")?.firstElementChild?.shadowRoot,
         stylesPanelP.promise,
         (stylesPanelWrapper) => {
...
class JsonStorage {
   ...
   createStructure(wrapper) {
      ...
      this.saveButton.className = "json-storage__button json-storage__button--save";
      this.saveButton.textContent = "Save";

      this.loadButton = document.createElement("button");
      this.loadButton.className = "json-storage__button json-storage__button--load";
...
async function init() {
   ...
   const classmapStorage = new JsonStorage(classmapStorageWrapper, "classmap", onClassmapChange);

   function patchElementsPanelSync(isRefresh) {
      const elementsPanel = main.querySelector('[aria-label="Elements panel"]');
      if (elementsPanel) {
         patchElementsPanel(elementsPanel, globalThis.MAP_CSS_CLASS, classmapStorageWrapper, isRefresh);
      }
   }
      ...
      for (const mutation of mutations) {
         if (mutation.addedNodes.length > 0) {
            ...
            if (element.ariaLabel === "Elements panel") {
               patchElementsPanel(element, globalThis.MAP_CSS_CLASS, classmapStorageWrapper);
            }
         }
      }
   });
